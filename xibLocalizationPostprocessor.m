...
int main(int argc, const char *argv[])
{
    NSAutoreleasePool *autoreleasePool = [[NSAutoreleasePool alloc] init]; {
        ...
        for (NSString *line in [rawXIBStrings componentsSeparatedByString:@"\n"]) {
            ...
            NSLog(@"Warning: skipped garbage input line %lu, contents: \"%@\"", lineCount, line);
        }
        
        if (outputStrings.length && ![outputStrings writeToFile:[NSString stringWithUTF8String:argv[2]] atomically:NO encoding:NSUTF16StringEncoding error:&error]) {
